import General from '@/constants/General'
import React from 'react'

const Activities = () => {
  return (
    <aside className='h-screen w-1/3 border-l bg-white z-0'>
      <div className='flex justify-between items-center px-3 h-16 border-b border-solid'>
        <h1 className="font-semibold text-xl">Notifications</h1>
      </div>

      <ul className="2xl:w-4/12 h-screen overflow-y-contain gap-y-4 top-0">
        {General.notifications.map(({ id, author, title, community, timestamp, icon: Icon }) => (
          <div key={id} className="w-full p-3 bg-white rounded flex">
            <div tabIndex={0} aria-label="heart icon" role="img" className="focus:outline-none w-8 h-8 border rounded-full border-gray-200 flex items-center justify-center">
              <svg width={16} height={16} viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M8.00059 3.01934C9.56659 1.61334 11.9866 1.66 13.4953 3.17134C15.0033 4.68334 15.0553 7.09133 13.6526 8.662L7.99926 14.3233L2.34726 8.662C0.944589 7.09133 0.997256 4.67934 2.50459 3.17134C4.01459 1.662 6.42992 1.61134 8.00059 3.01934Z" fill="#EF4444" />
              </svg>
            </div>
            <div className="pl-3">
              <p tabIndex={0} className="focus:outline-none text-sm leading-none">
                <span className="text-darkGreen">{author}</span> {title}{" "}
                <span className="text-darkGreen font-semibold">{community}</span>
              </p>
              <p tabIndex={0} className="focus:outline-none text-xs leading-3 pt-1 text-gray-500">{timestamp}</p>
            </div>
          </div>

        ))}
      </ul>
    </aside>
  )
}

export default Activities